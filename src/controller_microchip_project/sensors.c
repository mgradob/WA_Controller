...
{
	if (U1STAbits.OERR) { // 1 = Receive buffer has overflowed, 0 = Receive buffer has not overflowed
		U1STAbits.OERR = 0;
		return -2;
	}
	
	if (U1STAbits.URXDA) {//1 = Receive buffer has data, at least one more character can be read,0 = Receive buffer is empty
		*data = U1RXREG & 0x00FF;
		return 0;
	} else {
		*data = 0;
...
void Serial_Receive_Frame(unsigned char *ch, unsigned char len)
{
	unsigned char i;

	for (i = 0; i < len; i++) {
		while (!IFS0bits.U1RXIF);
		while( Serial_Receive(ch++) );
		IFS0bits.U1RXIF = 0;
	}
}
